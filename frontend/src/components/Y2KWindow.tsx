...
const Y2KWindow = ({ title, children, onClose, onBack, className = "", isStatic = false, contentClassName = "" }: Y2KWindowProps) => {
  const windowContent = (
    <div
        ...
        className={`bg-white/80 backdrop-blur-xl w-full border-2 border-white relative shadow-2xl flex flex-col ${!isStatic ? 'max-w-lg animate-in fade-in zoom-in duration-300 max-h-[90vh]' : ''} ${className}`}
    >
  ...
  return (
    <div
        className="fixed inset-0 z-50 flex items-start sm:items-center justify-center p-4 bg-black/50 backdrop-blur-sm cursor-pointer overflow-y-auto"
        onClick={onClose}
    >
