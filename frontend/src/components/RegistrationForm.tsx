...
const RegistrationForm = ({ onBack }: { onBack?: () => void }) => {
  ...
  const [verificationState, setVerificationState] = useState({
    step: 'initial', // initial, challenge, verified
    phoneHint: '',
    challengePhone: '',
    challengeEmail: '',
    error: ''
  ...
  const handleVerify = async () => {
    try {
      ...
      if (response.data.verified) {
        ...
        // Trigger municipality load if department is set
  ...
  const handleSubmit = async (e: FormEvent) => {
    ...
    try {
        if (verificationState.step === 'verified') {
             ...
             // Exclude referrer_code from update payload to avoid issues
      ...
      setFormData({
        ...
        department_id: '',
        municipio_id: '',
        referrer_code: formData.referrer_code // Keep referrer code if present
      });
      setReferrerName(''); // Clear referrer name display if we want, but maybe keep it if code is kept
  ...
  return (
    <div className="bg-secondary min-h-screen flex items-center justify-center p-4 font-sans relative">
      ...
      <Y2KWindow title="Registro" onBack={onBack} className="max-w-3xl">
            {isReferralValid ? (
            <div className="bg-pearl-aqua/20 border-2 border-pearl-aqua text-black p-5 mb-8 shadow-sm" role="alert">
            ...
            <div className="bg-white/50 border-2 border-primary text-black p-5 mb-8 shadow-sm" role="alert">
                <p className="font-bold text-lg">¡Bienvenido!</p>
            ...
            {Object.keys(fieldErrors).length > 0 && (
              <div className="bg-white/70 border-2 border-primary text-primary p-5 mb-8 shadow-sm" role="alert">
                <p className="font-bold text-lg uppercase mb-2">Por favor corrija los siguientes errores:</p>
            ...
            <form onSubmit={handleSubmit} className="space-y-6">
                ...
                {isReferralValid && (
                    <div>
                        <label className="block text-sm font-bold text-black mb-1 uppercase tracking-wider">Referenciado por</label>
                        <input 
                            type="text" 
                            value={referrerName} 
                            readOnly 
                            className="block w-full bg-white text-black border-2 border-black/10 focus:border-dark-periwinkle focus:ring-0 sm:text-sm p-3 cursor-not-allowed opacity-70"
                        />
                    </div>
                )}

            <div>
                <label className="block text-sm font-bold text-black mb-1 uppercase tracking-wider">Cédula</label>
                <input
                type="text"
                name="cedula"
                ...
                required
                className="block w-full bg-white text-black border-2 border-transparent focus:border-dark-periwinkle focus:ring-0 sm:text-sm p-3 placeholder-gray-500 shadow-inner"
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                <div>
                <label className="block text-sm font-bold text-black mb-1 uppercase tracking-wider">Nombres</label>
                <input
                    type="text"
                    name="nombres"
                    ...
                    required
                    className="block w-full bg-white text-black border-2 border-transparent focus:border-dark-periwinkle focus:ring-0 sm:text-sm p-3 shadow-inner"
                />
                </div>
                <div>
                <label className="block text-sm font-bold text-black mb-1 uppercase tracking-wider">Apellidos</label>
                <input
                    type="text"
                    name="apellidos"
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                <div>
                    <label className="block text-sm font-bold text-black mb-1 uppercase tracking-wider">Sexo</label>
                    <select
                        name="sexo"
                        value={formData.sexo}
                        onChange={handleChange}
                        required
                        className="block w-full bg-white text-black border-2 border-transparent focus:border-dark-periwinkle focus:ring-0 sm:text-sm p-3 shadow-inner appearance-none"
                    >
                ...
                <div>
                    <label className="block text-sm font-bold text-black mb-1 uppercase tracking-wider">Teléfono</label>
                    <input
                        type="tel"
                        name="phone"
            ...
            <div>
                <label className="block text-sm font-bold text-black mb-1 uppercase tracking-wider">Correo Electrónico</label>
                <input
                type="email"
                name="email"
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                <div>
                <label className="block text-sm font-bold text-black mb-1 uppercase tracking-wider">Departamento</label>
                <select
                    name="department_id"
                ...
                <div>
                <label className="block text-sm font-bold text-black mb-1 uppercase tracking-wider">Municipio</label>
                <select
                    name="municipio_id"
                    ...
                    className="block w-full bg-white text-black border-2 border-transparent focus:border-dark-periwinkle focus:ring-0 sm:text-sm p-3 shadow-inner disabled:bg-white/50 disabled:text-gray-400 appearance-none"
                >
                    <option value="">Seleccione...</option>
            ...
            <button
                ...
                className="w-full md:w-1/3 mx-auto rounded-full flex justify-center py-4 px-4 border-2 border-primary text-lg font-black text-primary bg-white hover:bg-primary hover:text-white hover:border-primary focus:outline-none focus:ring-0 disabled:opacity-50 disabled:cursor-not-allowed transition-colors duration-0 mt-8 uppercase tracking-widest shadow-md"
            >
                Registrar
            </button>
            
            {message && !successData && (
                <div className={`mt-6 p-4 border-2 ${message.includes('exitoso') ? 'bg-pearl-aqua/20 text-black border-pearl-aqua' : 'bg-racing-red/20 text-black border-white'}`}>
            ...
            <div className="bg-pearl-aqua/20 border-2 border-pearl-aqua text-black p-6 shadow-lg">
              ...
              <p className="text-sm mb-4 font-medium">
                {message.includes('Actualización') 
                  ? "Tus datos han sido actualizados correctamente. Comparte tu enlace para invitar a otros y hacer crecer la red:" 
