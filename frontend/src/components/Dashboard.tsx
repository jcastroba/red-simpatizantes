  ...
  return (
    <div className="bg-secondary min-h-screen p-2 sm:p-4 font-sans relative">
      <div className="max-w-6xl mx-auto space-y-4 sm:space-y-6">
        <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-3 bg-white/80 backdrop-blur-md p-3 sm:p-4 border-2 border-white shadow-lg">
            <div>
            ...
            <button
                onClick={onLogout}
                className="bg-racing-red text-white font-bold px-3 sm:px-4 py-2 border-2 border-white hover:bg-red-700 transition-colors uppercase text-xs sm:text-sm w-full sm:w-auto"
            >
                Cerrar Sesión
            </button>
        </div>
            ...
            <div className="md:col-span-1">
                <Y2KWindow title="Estadísticas" className="h-full" isStatic={true}>
                    <div className="text-center py-4 sm:py-8">
                        <p className="text-xs sm:text-sm font-bold uppercase text-black/60 mb-2">Total Referidos</p>
                        <p className="text-4xl sm:text-6xl font-black text-primary drop-shadow-sm">{data.referrals_count}</p>
                    </div>
                </Y2KWindow>
            </div>
            ...
            <div className="md:col-span-2">
                <Y2KWindow title="Tu Link de Referido" className="h-full" isStatic={true}>
                    <div className="space-y-3 sm:space-y-4">
                        <p className="text-xs sm:text-sm text-black">Comparte este enlace para invitar a más personas a tu red:</p>
                ...
                <Y2KWindow title="Mis Referidos" className="h-full" isStatic={true}>
                    <div className="mb-3 sm:mb-4">
                        <input
                            ...
                            className="w-full p-2 border-2 border-black/10 focus:border-hot-fucsia outline-none text-xs sm:text-sm"
                        />
                    </div>
                        ...
                        <>
                            ...
                            <div className="sm:hidden space-y-3 max-h-96 overflow-y-auto">
                                {data.referrals
                                    ...
                                    <div key={index} className="bg-black/5 p-3 border border-black/10">
                                        <div className="flex justify-between items-start mb-2">
                                            <div>
                                                <p className="font-bold text-sm text-black">{ref.nombres} {ref.apellidos}</p>
                                                <p className="text-xs text-black/60">{ref.cedula || 'Sin cédula'}</p>
                                            </div>
                                            <span className="bg-primary text-white text-xs px-2 py-1 font-bold">
                                                {ref.referrals_count} ref.
                                            </span>
                                        </div>
                                        <button
              ...
              <div className="space-y-4 sm:space-y-6">
                  <div className="space-y-3 sm:space-y-4">
                      ...
                      <div className="grid grid-cols-2 gap-3 sm:gap-4">
                          <div>
                              <p className="font-bold text-black/60 text-[10px] sm:text-xs uppercase">Cédula</p>
                              <p className="text-sm">{selectedReferral.cedula}</p>
                          </div>
                          <div>
                              <p className="font-bold text-black/60 text-[10px] sm:text-xs uppercase">Teléfono</p>
                              <p className="text-sm">{selectedReferral.phone}</p>
                          </div>
                      </div>
                      <div>
                          <p className="font-bold text-black/60 text-[10px] sm:text-xs uppercase">Correo Electrónico</p>
                  ...
                  <div className="border-t-2 border-black/10 pt-3 sm:pt-4">
                      <h3 className="font-bold text-black uppercase text-xs sm:text-sm mb-3 flex justify-between items-center">
                          ...
                          <span className="bg-hot-fucsia text-white text-[10px] sm:text-xs px-2 py-0.5 rounded-full">{selectedReferral.referrals_count}</span>
                      </h3>
                      ...
                      {selectedReferral.sub_referrals && selectedReferral.sub_referrals.length > 0 ? (
                          ...
                          <p className="text-xs sm:text-sm text-black/40 italic">Este usuario aún no tiene referidos.</p>
                      )}
                  </div>
              </div>
          </Y2KWindow>
